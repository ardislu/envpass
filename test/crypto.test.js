...
suite('decryptRaw', { concurrency: true }, () => {
  ...
  test('throws on invalid magic in value', { concurrency: true }, async () => {
    // @ts-expect-error
  ...
  test('throws on invalid version in value', { concurrency: true }, async () => {
    // @ts-expect-error
    await rejects(decryptRaw({ prf, value: 'envpass:INVALID:AAA' }), { name: 'TypeError', message: 'envpass version "INVALID" is not supported.' });
  });
  test('throws on invalid (non-base64) string in value', { concurrency: true }, async () => {
